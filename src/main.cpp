 ...
 *
 * Responsabilidades:
 * 1. Ler o nome do arquivo de configuração da linha de comando.
 * 2. Instanciar e configurar o Simulador com os parâmetros lidos.
 * 3. Calcular a rota para cada pacote e agendar seus eventos iniciais.
 * 4. Agendar o primeiro evento de transporte.
 * 5. Iniciar a execução da simulação.
 * 6. Limpar a memória alocada (matriz de adjacência).
 */
int main(int argc, char* argv[]) { // CORRIGIDO: Adicionada a chave { para iniciar a função

    // CORRIGIDO: Adicionada verificação crucial para o número de argumentos.
    // Isso evita que o programa quebre se for executado sem um nome de arquivo.
    if (argc != 2) {
    ...
    if (!inputFile.is_open()) {
        // CORRIGIDO: Mensagem de erro agora usa a variável para informar qual arquivo falhou.
